...
public function linkedin(Request $request){
    ...
    public function Paitentregister(Request $request){
        try{
            ...
            $Input['isactivationcomplete'] = '0';
            $Input['patient_id'] = '';
            $Input['temporary_patient_id'] = trim($request->temporary_patient_id);
            $Input['logins'] = '0';
            $Input['created_by'] = '';
            $Input['updated_by'] = '';
            ...
            if($auth_user) {
                ...
                $email = $request->email;
                $user_device = $user['device'];
                $temp_patient_id = $user['temporary_patient_id'];
                if($user_device=='mobile'){
                    $old_active_code = $user['active_code'];
                    $active_code = substr($old_active_code, -6);
                } else {
                ...
                $sendemail = Mail::to($email)->send(new SendActivationCode($fullname,$email,$active_code,$user_device,$temp_patient_id));
    ...
    {   
        try{
            //$check_login_details = User::where('username', $request->patientid)->orWhere('patient_id', $request->patientid)->where('password', $request->password)->first();
            $check_login_details = User::where(function($query) use ($request) {
                                            $query->where('username', $request->patientid)
                                                  ->orWhere('patient_id', $request->patientid);
                                            })
                                    ->where('hdpwd', $request->password)
                                    ->first();
            if($check_login_details){
                return response()->json(['status'=>true,'message'=>'user data found','error'=>'credentials match','data'=>$check_login_details]);
            } else {
                return response()->json(['status'=>false,'message'=>'Please Enter Valid Password or Username','error'=>'Invalid credentials','data'=>$check_login_details]);
            }
        } catch(\Illuminate\Database\QueryException $e) {
            $errorClass = new ErrorsClass();
            $errors = $errorClass->saveErrors($e);
