...
public function UpdatePatientId(Request $request){
  try{
      $update_data = User::where('id', $request->userid)->update(['patient_id' => $request->patient_id]);
      if($update_data) {
          return response()->json(['status'=>true,'message'=>'Patient Id updated successfully!','error'=>'', 'data'=>'']);
      } else {
          return response()->json(['status'=>false,'message'=>'Sorry fail to update patient id. Please try again.','error'=>'','data'=>'']); 
      }
  } catch(\Illuminate\Database\QueryException $e) {
      $errorClass = new ErrorsClass();
      $errors = $errorClass->saveErrors($e);
