...
class PackagesController extends Controller
{
    ...
    public function CreatePackage(Request $request)
    {
    	try{
        	...
        	if($request->file('package_banner_image')){
            	$file = $request->file('package_banner_image');
            	$filename = date('YmdHi').$file->getClientOriginalName();
            	$file-> move(public_path('PackagesBannerimg'), $filename);
            	$package->banner_image = $filename;
        	}
    ...
    public function UpdatePackage(Request $request, $id)
    {
    	try{
        	...
        	if($request->file('edit_package_banner_image')){
            	$file = $request->file('edit_package_banner_image');
            	$filename = date('YmdHi').$file->getClientOriginalName();
            	$file-> move(public_path('PackagesBannerimg'), $filename);
            	$package['banner_image'] = $filename;
        	}
    ...
    public function GetSinglePackageData(Request $request, $name)
    {
    	try{
    		if(is_numeric($name)){
    			$single_package_data = Packages::select('pre_existing_conditon.name as package_pre_condition', 'department.department_name as department_name', 'packages.*')->join('pre_existing_conditon', 'packages.pre_existing_conditon_id', '=', 'pre_existing_conditon.id')->join('department', 'packages.dept_id', '=', 'department.dept_id')->where('packages.id', $name)->first();
    		} else {
    			$single_package_data = Packages::select('pre_existing_conditon.name as package_pre_condition', 'department.department_name as department_name', 'packages.*')->join('pre_existing_conditon', 'packages.pre_existing_conditon_id', '=', 'pre_existing_conditon.id')->join('department', 'packages.dept_id', '=', 'department.dept_id')->where('packages.name', 'like', '%' .trim($name). '%')->first();
    		}
